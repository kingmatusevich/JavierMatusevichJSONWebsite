...
<html>
	<head>
		...
		$(document).ready(function(){
			...
			function jobscounted(nn, tip){
			if ((tip > nn) && ((window.rclic == true) || (window.frun == false))){
						console.log("tip "+tip);
						window.frun = true;
						window.rclic = false;
						var nNumber = parseInt(nn)+1;
						var rNumber = parseInt(nNumber);
						$("#right_arrow").attr('rel', 'address:/jobs/'+rNumber);
						$("#right_arrow").fadeIn('slow');
						$(".arrow_container").fadeIn('slow');
						} else {
						$("#right_arrow").fadeOut('slow');
						}
						if((nn > 0) && ((window.lclic == true) || (window.frun == false))){
						window.lclic = false;
						var pNumber = parseInt(nn)-1;
						var kNumber = parseInt(pNumber);
						$("#left_arrow").attr('rel', 'address:/jobs/'+kNumber);
			...
			function countjobs(aNumber){
				$.ajax({ 
				data: "", 
				type: "GET", 
				dataType: "json", 
				url: "API/json/jobs.list.json", 
				success: function(data){ 
					window.aJobs = data.last;
					console.log("datalast "+data.last+"aNumber "+aNumber);
					jobscounted(aNumber, data.last);
				} 
				});
			}
			...
			if ($(window).width() < 1122){
				if ($(window).width() >= 982){
					$("#hint").css('background-image', 'url(pic/hint-alt.png)');
				} else {
					$("#hint").css('background-image', 'url(pic/hint-sm.png)');
				}
			}
			$(window).resize(function(){
			if ($(window).width() < 1122){
				if ($(window).width() >= 982){
					if($("#hint").css('background-image') != 'url(pic/hint-alt.png)'){
						$("#hint").css('background-image', 'url(pic/hint-alt.png)');
					}
					 
				} else {
					if($("#hint").css('background-image') != 'url(pic/hint-sm.png)'){
						$("#hint").css('background-image', 'url(pic/hint-sm.png)');
					}
				}
			} else {
				if($("#hint").css('background-image') != 'url(pic/hint.png)'){
						$("#hint").css('background-image', 'url(pic/hint.png)');
					}
			}
			
			
			})
			$("#menu").draggable({handle: '.preimage' , cursor: 'move', scroll: false, containment : 'document', 
			start: function(){
			...
			$(".arrow_container").draggable({handle: '.preimage' , cursor: 'move', scroll: false, containment : 'document', 
			start: function(){
				$("#hint").fadeOut('slow', function(){
				$("#hint").hide();
				});
			...
			$("#banner").draggable({cursor: 'move', scroll: false,   
			start: function(){
				$("#hint").fadeOut('slow', function(){
				$("#hint").hide();
				});
			...
			$("#content").draggable({handle: '#content_title', cursor: 'move', scroll: false, containment : 'document', 
			start: function(){
				$("#hint").fadeOut('slow', function(){
				$("#hint").hide();
				});
			...
			$(".option").click(function(){
				$(".selected").removeClass("selected").addClass("option"); 
			...
			$.address.internalChange(function(res){
				 console.log('internal');
				if (res.path.toLowerCase().indexOf("jobs/") >= 0) {
					
					var Number = res.path.replace('jobs/', '');
					console.log(Number);
					Number = Number.replace('/', '');
					if(!window.aJobs){
						countjobs(Number);
					} else {
						if (window.aJobs > Number){
						var nNumber = parseInt(Number)+1;
						var xNumber = parseInt(nNumber);
						$("#right_arrow").attr('rel', 'address:/jobs/'+xNumber);
						$("#right_arrow").fadeIn('slow');
						$(".arrow_container").fadeIn('slow');
						} else {
						$("#right_arrow").fadeOut('slow');
						...
						if(Number > 0){
						var pNumber = parseInt(Number)-1;
						var zNumber = parseInt(pNumber);
						$("#left_arrow").attr('rel', 'address:/jobs/'+zNumber);
						$("#left_arrow").fadeIn('slow');
						$(".arrow_container").fadeIn('slow');
						} else {
						$("#left_arrow").fadeOut('slow');
			...
			$.address.externalChange(function(res){
				console.log('external');
				if(res.path.toLowerCase().indexOf("jobs/") >= 0){
					var Number = res.path.replace('jobs/', '');
					console.log(Number);
					Number = Number.replace('/', '');
					console.log("window.aJobs="+window.aJobs);
					if(!window.aJobs){
					countjobs(Number);
					} else{
						if (window.aJobs > Number){
						var nNumber = parseInt(Number)+1;
						var mNumber = parseInt(nNumber);
						$("#right_arrow").attr('rel', 'address:/jobs/'+mNumber);
						$("#left_arrow").fadeIn('slow');
						$(".arrow_container").fadeIn('slow');
						};
						if(Number > 0){
						var pNumber = parseInt(Number)-1;
						var asdNumber = parseInt(pNumber);
						$("#right_arrow").attr('rel', 'address:/jobs/'+asdNumber);
						$("#right_arrow").fadeIn('slow');
						$(".arrow_container").fadeIn('slow');
						};	
					} 
					...
					$(".selected").removeClass("selected").addClass("option"); 
					$("#jobs").removeClass("option").addClass("selected");  
				} else {
					var ID = $("#"+res.path.replace('/', '')).attr('id');
					console.log("done:"+ID); 
					if(!ID) {ID = 'welcome'};
					$('#'+ID).trigger('click');
				}
		...
		<meta name="description" content="Javier Matusevich es un diseñador Web de Mar del Plata, Mac User y developer de iOS. Haz clic para visitar su sitio web">
